...
export default function LoginPage() {
 ...
 useEffect(()=>{api('/config').then(setConfig).catch(()=>setError('Could not load sign-in settings. Refresh to try again.'));},[]);
 ...
 const submit=async(e:React.FormEvent)=>{
  ...
  try{
   ...
   if(sent){const session=await verifyOtp(normalizedPhone,code,language);accept(session);navigate(session.user.role==='expert'?'/expert':'/');}
  ...
  <div className="login-content">
   ...
   <div className="login-form-box">
    ...
    <form onSubmit={submit}>
     ...
     <Button data-testid="login-submit" className="btn primary full" type="submit" disabled={busy||!config||config.otp_mode==='unconfigured'}>{busy?<Loader2 className="spin" size={17}/>:<>{sent?t('Verify & continue','सत्यापित करें','ਤਸਦੀਕ ਕਰੋ'):t('Get verification code','सत्यापन कोड प्राप्त करें','ਤਸਦੀਕ ਕੋਡ ਪ੍ਰਾਪਤ ਕਰੋ')}<ArrowRight size={17}/></>}</Button>
