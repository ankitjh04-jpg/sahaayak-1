...
from .config import FARMER_PHONE, EXPERT_PHONE, DEMO_EXPERT_ACCOUNTS
...
async def seed():
    ...
    expert = None
    for index, (email, password, name) in enumerate(DEMO_EXPERT_ACCOUNTS):
        existing = await db.users.find_one({'email': email}, {'_id': 0})
        if not existing and index == 0:
            existing = await db.users.find_one({'phone': EXPERT_PHONE}, {'_id': 0})
        if not existing:
            existing = {'id': f'demo-expert-{index}', 'created_at': stamp()}
            if index == 0:
                existing['phone'] = EXPERT_PHONE
            await db.users.insert_one(existing.copy())
        await db.users.update_one({'id': existing['id']}, {'$set': {'email': email, 'role': 'expert', 'language': 'en', 'password_hash': hash_password(password), 'disabled': False, 'profile': {'name': name, 'location': 'Ludhiana, Punjab'}, 'updated_at': stamp()}})
        if index == 0:
            expert = {**existing, 'email': email, 'role': 'expert', 'profile': {'name': name, 'location': 'Ludhiana, Punjab'}}
