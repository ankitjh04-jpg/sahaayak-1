...
export const FarmMap = ({fields=EMPTY_FIELDS,onSelect,selected}: Props) => {
 ...
 if(config?.google_maps_browser_key&&!failed)return <GoogleFarmMap apiKey={config.google_maps_browser_key} mapId={config.google_maps_map_id} fields={fields} onSelect={onSelect} selected={selected} onFailure={()=>setFailed(true)}/>;
 return <div><LeafletFarmMap fields={fields} onSelect={onSelect} selected={selected}/><p className="map-provider-note" data-testid="map-provider-status">{failed?'Google Maps unavailable. Showing OpenStreetMap.':'OpenStreetMap · Google Maps activates when its browser key is configured.'}</p></div>;
};
